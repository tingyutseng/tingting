...
class IChingLogic:
    ...
    def _evaluate_static_risk(self, u_elem, l_elem):
        # 判斷五行關係
        relation = self.element_relations.get((u_elem, l_elem), self.element_relations.get((l_elem, u_elem)))
        
        if relation is None:
             # 無特定關係，或數據錯誤
             return "中性", "穩定中性 (萬物靜觀皆自得)", "info", "無特定關聯"
    ...
    def _generate_ai_decision_insight(self, hex_data, upper, lower, risk_desc, theme, u_ctx, l_ctx, is_se_ying_conflict, elem_relation):
        ...
        if risk_desc.startswith("高度協調"):
            opening = f"**「天助自助，相生共榮。」** 您處於【{tag}】的時空，內在與外在的氣場高度協調。此時機利於把握順流，事半功倍，但仍需智慧引導。"
        elif risk_desc.startswith("結構衝突"):
            opening = f"**「時與我爭，逆境方顯真英雄。」** 您正處於【{name}】的嚴峻考驗。環境與心態存在本質牴觸，充滿挑戰和摩擦，需耗費大量心力來平衡，務必採取保守策略。"
        ...
        # 2. 氣場與情境分析 (氣場警示 + 情境描述)
        element_line = f"**【時空氣場分析】** 外在 **{upper['element']}** ({upper['name']}) 與內在 **{lower['element']}** ({lower['name']}) 呈 **{elem_relation}**。"
        context_line = f"當前您的外在趨勢為：*{u_ctx}*；內在心態為：*{l_ctx}*。"
        
        # 3. 關係與核心行動原則 (關係建議)
        # 根據世應和風險協調度，給出最終建議。
        if '相斥' in is_se_ying_conflict and risk_desc.startswith("高度協調"):
            action_principle = "雖然氣場協調，但世應相斥，您必須主動與外部建立互補，不能單憑一己之力。"
        elif '重疊' in is_se_ying_conflict and risk_desc.startswith("結構衝突"):
            action_principle = "世應重疊且氣場衝突，應當**立即退守**，將精力集中在最核心的基礎上，避免雙重消耗。"
        elif risk_desc.startswith("能量疊加"):
            action_principle = "您的行動力極強，核心原則應是：**在行動前多加審視，知進知退，方能持盈保泰。**"
        else:
    ...
    if analyze_btn:
        with st.spinner(f"正在分析主題【{theme_sel}】下的時空組合..."):
            ...
            with col_info:
                 ...
                 st.metric("時空氣場協調度", risk_score, elem_relation) # 協調度簡化顯示
                 st.markdown("##### 卦象簡述")
