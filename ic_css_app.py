...
class IChingLogic:
    ...
    def get_hexagram_data(self, theme, upper_id, lower_id):
        ...
        # 獲取卦象文本
        hex_data = self.hexagram_data.get(hex_code, {"name": f"上{upper['name']}下{lower['name']}", "tag": "數據缺失", "gua_ci": "此卦辭數據缺失。", "yao_ci": {}})
...
def main():
    ...
    st.caption("最終靜態版：8 大主題 x 64 卦 | 數據演示中")
    st.markdown("---")
    ...
    # 為了讓「乾為天」的演示更方便，這裡設置乾卦的 index=0
    ...
    if analyze_btn:
        with st.spinner(f"正在分析主題【{theme_sel}】下的時空組合..."):
            ...
            with res_c2:
                ...
                st.metric("五行判斷風險", risk)
                
                st.markdown("##### 情境解讀")
                st.info(f"**在【{theme_sel}】主題下：**")
                st.markdown(f"- **外在環境 (上卦)**：**{upper['symbol']} {upper['name']}** - *{u_ctx}*")
                st.markdown(f"- **內在心態 (下卦)**：**{lower['symbol']} {lower['name']}** - *{l_ctx}*")

            st.markdown("---")

            # 4. 周易原文輸出
            st.subheader("📚 周易經典原文與決策指引")
            ...
            if yao_ci:
                ...
                for i in range(6, 0, -1): # 從上爻(6)往初爻(1)倒序顯示，更符合卦象閱讀習慣
                    ...
                    st.markdown(
                        f'<div class="yao-line">**[{pos_name} - {yao_pos}]** {yao_text}</div>', 
                        unsafe_allow_html=True
                    )
            else:
